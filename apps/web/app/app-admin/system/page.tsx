...
const systemLimitsSchema = z.object({
  maxTransferCents: z.coerce
    .number()
    .int()
    .positive('Must be positive')
    .optional(),
  maxTopupCents: z.coerce
    .number()
    .int()
    .positive('Must be positive')
    .optional(),
...
export default function AdminSystemPage() {
  ...
  const form = useForm<SystemLimitsForm>({
    resolver: zodResolver(systemLimitsSchema) as Resolver<SystemLimitsForm>,
    defaultValues: {
  ...
  const updateMutation = useMutation({
    mutationFn: (body: Partial<SystemLimits>) =>
      adminApi.system.patchLimits(body),
    onSuccess: () => {
  ...
  function handleSubmit(values: SystemLimitsForm) {
    ...
    if (values.maxTransferCents != null)
      patch.maxTransferCents = values.maxTransferCents;
    if (values.maxTopupCents != null)
      patch.maxTopupCents = values.maxTopupCents;
    if (values.dailyLimitCents != null)
      patch.dailyLimitCents = values.dailyLimitCents;
    if (values.monthlyLimitCents != null)
      patch.monthlyLimitCents = values.monthlyLimitCents;
